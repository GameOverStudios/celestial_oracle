d3.json("/categorias_json", function (error, data) {
  if (error) throw error;
  //var data = { "name": "A1", "children": [{ "name": "B1", "children": [{ "name": "C1", "value": 100 }, { "name": "C2", "value": 300 }, { "name": "C3", "value": 200 }] }, { "name": "B2", "value": 200 }] };
  var root = d3.hierarchy(data);
  //console.log(data);

  
  // Definir o tamanho do gráfico de forma mais controlada (proporcional à tela)
  const width = (window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth) * 0.9;  // 90% da largura da tela
  const height = (window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight) * 0.9; // 90% da altura da tela
  ...
  */
  ...
  var treemapNodes = d3.select("#map g")
    .selectAll("g")
    .data(root.descendants())
    .enter()
  ...
  // Função para mostrar a visualização selecionada
  function showVisualization() {
    var selectedValue = document.getElementById("visualization-select").value;
    var wrappers = document.querySelectorAll(".wrapper");

    wrappers.forEach(function (wrapper) {
      // Corrigido: Comparar com o valor selecionado diretamente
      wrapper.style.display = wrapper.id.includes(selectedValue) ? "flex" : "none";
    });
  }

  // Adiciona um ouvinte de evento ao menu suspenso
  document.getElementById("visualization-select").addEventListener("change", showVisualization);

  // Mostra a primeira visualização por padrão
  showVisualization();
